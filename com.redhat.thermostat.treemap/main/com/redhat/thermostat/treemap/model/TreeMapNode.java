...
import com.redhat.thermostat.treemap.impl.Decorator;
import com.redhat.thermostat.treemap.impl.SquarifiedTreeMap;
 ...
 */
public class TreeMapNode implements Comparable<TreeMapNode>, Decorator {
     ...
     * 
     */
    public TreeMapNode(int realWeight) {
        this(null, realWeight);
    }
     ...
     * 
     */
    public TreeMapNode(String label, int realWeight) {
        this.id = idCounter++;
        this.label = (label == null || label.equals("")) ? Integer.toString(id) : label;
        this.parent = null;
    ...
    public String toString() {
        return "TreeMapNode [" + "id = " + id + 
                "; weight=" + weight +
                "; [rectangle=" + rectangle + "]";
    }

    /**
     * Recursively print the subtree nested in this object.
     */
    ...
    /**
     * Print this object and call recursively this method on its children.
     * @param root the node to print.
     * @param level the nested level of this node.
     */
    private void printTree(TreeMapNode root, int level) {
        String space = "";
        for (int i=0; i<level; i++){
            space += "  ";
        }
        space += "|->";
        System.out.println(space + "[ " + root.label + ": realWeight = " + root.getRealWeight() + " ]");
    ...
    /**
     * Compare this object with the one given in input.
     * @return a positive number if this object is greater than the given one, 
     * 0 if they are equal, else a negative number.
     */
    ...
    /**
     * This method sorts the given list in descending order using the quick sort
     * algorithm.
     * @param nodes the list of {@link TreeMapNode} to sort.
     * @param start the sorting left index.
     * @param end the sorting end index.
     */
    public static void quickSort(List<TreeMapNode> nodes, int start, int end) {
        int i = start;
        int j = end;
        TreeMapNode temp;
        TreeMapNode middle = nodes.get((start + end) / 2);

        while (i < j) {
            while (nodes.get(i).compareTo(middle) > 0) {
                i++;
            }
            while (nodes.get(j).compareTo(middle) < 0) {
                j--;
            }
            if (j >= i) {
                temp = nodes.get(i);
        ...
        if (start < j) {
            quickSort(nodes, start, j);
        }
        if (i < end) {
            quickSort(nodes, i, end);
        }
    }
